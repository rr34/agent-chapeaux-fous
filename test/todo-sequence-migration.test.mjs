...
const recognitionMigration = readMigrationLedger(path.join(root, "db", "migrations.sql"))
  .find(({ version }) => version === 17);
...
test("migration 17 recognizes groups that already contain sequenced tasks", () => {
  assert.ok(migration);
  assert.ok(recognitionMigration);
  const database = new DatabaseSync(":memory:");
  try {
    database.exec(`
      CREATE TABLE todo_groups (
      ...
      INSERT INTO todo_groups (todo_group_id, name)
      VALUES (1, 'Ordinary'), (2, 'Existing numbered work');
      INSERT INTO personal_tasks (todo_group_id, text)
      VALUES (1, 'No number'), (2, 'Unnumbered history');
      INSERT INTO personal_tasks (todo_group_id, sequence, text)
      VALUES (2, 41, 'Existing number');
    `);
    database.exec(migration.sql);
    database.exec(recognitionMigration.sql);

    assert.deepEqual(
      database.prepare("SELECT uses_sequence FROM todo_groups ORDER BY todo_group_id")
        .all().map(({ uses_sequence }) => uses_sequence),
      [0, 1],
    );
    assert.equal(
      database.prepare("SELECT sequence FROM personal_tasks WHERE text = 'Unnumbered history'").get().sequence,
      42,
    );
    database.prepare(`
      INSERT INTO personal_tasks (todo_group_id, text) VALUES (2, 'Gets next number')
    `).run();
    assert.equal(
      database.prepare("SELECT sequence FROM personal_tasks WHERE text = 'Gets next number'").get().sequence,
      43,
    );
  } finally {
    database.close();
