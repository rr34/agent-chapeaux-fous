...
test("a non-temporal to-do links directly to a guide and keeps the link when completed", async (context) => {
  const { registry } = harness(context);
  ...
  assert.equal(createdTodo.task.interaction_guide.name, "Evening Reflection");

  const unlinked = await registry.execute("todo_interaction_guide_set", {
    personal_task_id: createdTodo.task.personal_task_id,
    interaction_guide_id: null,
  });
  assert.equal(unlinked.task.interaction_guide, null);
  const relinked = await registry.execute("todo_interaction_guide_set", {
    personal_task_id: createdTodo.task.personal_task_id,
    interaction_guide_id: guideId,
  });
  assert.equal(relinked.task.interaction_guide.name, "Evening Reflection");
  ...
  assert.equal(completed.items[0].task.interaction_guide.interaction_guide_id, guideId);
  assert.equal(completed.items[0].task.interaction_guide.name, "Evening Reflection");
});

test("one-time to-dos can link an interaction guide", async (context) => {
  const { registry } = harness(context);
  const createdGuide = await registry.execute("interaction_guide_create", {
    name: "Weekly Review",
  });
...
test("the organizer API exposes a direct guide link independently of calendar placement", (context) => {
  const { store, guides } = harness(context);
  ...
  assert.equal(updated.text, "Plan tomorrow");
  assert.equal(updated.interactionGuideId, guide.id);
});
