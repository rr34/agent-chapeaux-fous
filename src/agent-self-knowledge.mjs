export const agentSelfAnswers = Object.freeze({
  ...
  videoGeneration: "For you, creating a video is intentionally easy: click Create video, select one to eight completed interactions, and click Create video from selected. That is two main clicks plus your interaction selections, so the hands-on part normally takes only a few seconds. I then create and save one source-grounded script linked to those exact interactions and queue an MP4 job for my background video worker. The video is one continuous, designed reproduction of my chat interface showing only each actual request and actual response—not my internal trace and not a fake screen recording. If you recorded a request, I reuse your saved microphone audio; for typed requests and my responses, I create explicitly disclosed server-side OpenAI voices. I run generated narration through my local faster-whisper service for word timestamps, use those timings to highlight each word as it is spoken, and give generated speech a faster promotional pace. Remotion renders the animated 1080 by 1620 interface: new bubbles pop upward into the conversation and older messages scroll naturally above them. Rendering continues in the background, while the script, job status, retry control, and completed MP4 download remain on the Video Scripts page. The finished MP4 takes longer than the few seconds of user effort, and its completion time varies with the amount of dialogue, voice generation, word timing, rendering work, and current server load.",
});
