...
personal-journal tools as the authoritative write and read path. Preserve each
observation as complete natural-language journal content. A tracker owns one
...
exists, call `journal_add` with `create_if_missing` false to return an unrecorded
...
When the user asks to correct existing observations, call `journal_list` to resolve
the exact stable `journal_entry_id`, then call `journal_update` on each intended entry.
